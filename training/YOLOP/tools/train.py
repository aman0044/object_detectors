...
from lib.utils import (DataLoaderX, is_parallel, run_anchor,
...
def main():
    ...
    if rank in [-1, 0]:
        ...
        if os.path.exists(cfg.MODEL.PRETRAINED_DET):
            logger.info(
                "=> loading model weight in det branch from '{}'".format(
        ...
        if cfg.TRAIN.DET_ONLY:  # Only train detection branch
            ...
            for k, v in model.named_parameters():
                ...
                if (
                    k.split(".")[1]
                    in Encoder_para_idx + Da_Seg_Head_para_idx + Ll_Seg_Head_para_idx
                ):
                    print("freezing %s" % k)
                    v.requires_grad = False
        ...
        if (
            cfg.TRAIN.ENC_DET_ONLY or cfg.TRAIN.DET_ONLY
        ):  # Only train encoder and detection branchs
        ...
        if cfg.TRAIN.LANE_ONLY:
            ...
            for k, v in model.named_parameters():
                ...
                if (
                    k.split(".")[1]
                    in Encoder_para_idx + Da_Seg_Head_para_idx + Det_Head_para_idx
                ):
                    print("freezing %s" % k)
                    v.requires_grad = False
        ...
        if cfg.TRAIN.DRIVABLE_ONLY:
            ...
            for k, v in model.named_parameters():
                ...
                if (
                    k.split(".")[1]
                    in Encoder_para_idx + Ll_Seg_Head_para_idx + Det_Head_para_idx
                ):
                    print("freezing %s" % k)
                    v.requires_grad = False
