...
def parse_args():
    ...
    parser.add_argument(
        "--iou_thres", type=float, default=0.6, help="IOU threshold for NMS"
    )
...
def main():
    ...
    # DP mode
    device = (
        select_device(logger, batch_size=cfg.TEST.BATCH_SIZE_PER_GPU * len(cfg.GPUS))
        if not cfg.DEBUG
    ...
    da_segment_results, ll_segment_results, detect_results, total_loss, maps, times = (
        validate(
            epoch,
            cfg,
            valid_loader,
